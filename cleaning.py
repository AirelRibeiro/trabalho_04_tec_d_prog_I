...
# Converte colunas em snake_case # david
def preprocess_columns(df):
    return [unidecode(col.lower().strip()) for col in df.columns]

# Formata IDs com 3 caracteres # david
def preprocess_dataframe(df):
    df["id"] = df["id"].astype(str).str.zfill(3)
    df["quantidade"] = pd.to_numeric(df["quantidade"].fillna(0), errors='coerce')
